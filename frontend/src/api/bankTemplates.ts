...
export async function listBankTemplates(companyId?: string): Promise<BankTemplate[]> {
  ...
  const response = await apiClient.get<BankTemplate[]>('/api/bank-templates', { params });
  return response.data;
}

export async function getBankTemplate(id: string): Promise<BankTemplate> {
  const response = await apiClient.get<BankTemplate>(`/api/bank-templates/${id}`);
  return response.data;
}
...
export async function createBankTemplate(payload: {
  ...
  const response = await apiClient.post<BankTemplate>('/api/bank-templates', payload);
  return response.data;
}
...
export async function createBankTemplateVersion(
  ...
  const response = await apiClient.post<BankTemplate>(`/api/bank-templates/${id}/versions`, version);
  return response.data;
}

export async function listBankTemplateVersions(id: string): Promise<BankTemplateVersion[]> {
  const response = await apiClient.get<BankTemplateVersion[]>(`/api/bank-templates/${id}/versions`);
  return response.data;
}
...
export async function detectBankTemplate(sourceFileId: string): Promise<DetectResult> {
  const response = await apiClient.post<DetectResult>('/api/bank-templates/detect', {
