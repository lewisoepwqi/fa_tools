...
export async function listJournalTemplates(companyId?: string): Promise<JournalTemplate[]> {
  ...
  const response = await apiClient.get<JournalTemplate[]>('/api/journal-templates', { params });
  return response.data;
}

export async function getJournalTemplate(id: string): Promise<JournalTemplate> {
  const response = await apiClient.get<JournalTemplate>(`/api/journal-templates/${id}`);
  return response.data;
}
...
export async function createJournalTemplate(payload: {
  ...
  const response = await apiClient.post<JournalTemplate>('/api/journal-templates', payload);
  return response.data;
}
...
export async function createJournalTemplateVersion(
  ...
  const response = await apiClient.post<JournalTemplate>(
    `/api/journal-templates/${id}/versions`,
    version
  );
  return response.data;
}
...
export async function setJournalTemplateStatus(
  ...
  const response = await apiClient.patch<JournalTemplate>(
    `/api/journal-templates/${id}/status`,
    null,
    { params: { status } }
  );
  return response.data;
