...
> - **§5 前端页面缺口现状**：#5/#6/#7/#8 模板/映射/规则管理页**已是完整增删改 + 版本 + 拖拽**（非「只读」）；#10 人工确认页已接通（W3 期）；#3 上传页公司选择器已修。剩 #4 模板识别确认页（依赖引擎类 detect 深化）、#11 导出记录独立页（价值低，批次详情已含导出）未做，归后续。
> - **仍归 W4**：仅数据运维（迁移契约/PG 实测/异步）+ P0-2 批次快照版本 ID（转换管道改造）；P1-3 去重·余额连续性是否纳入由 W4 brainstorm 与用户确认。
>
